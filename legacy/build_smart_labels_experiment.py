...
"""
Smart pair generator for the blind-spot experiment.

How it works:
- Reads processed vacancies.csv and resumes_experiment.csv
- Computes a rough relevance score for each vacancy-resume pair
- Preserves candidate_id and version_type from experimental resumes
- Deduplicates selection by candidate_id (if present), so one candidate does not occupy
  multiple slots with different resume versions inside the same bucketed sample
...
DEFAULT_CONFIG = {
    ...
    "resumes_csv": "processed/resumes_experiment.csv",
    "annotations_dir": "annotations",
    "reports_dir": "reports",
    "output_labels_file": "labels_template_smart_experiment.csv",
    "output_report_file": "pair_selection_report_experiment.csv",
    "pairs_per_vacancy": 12,
    "top_k": 4,
    "borderline_k": 4,
    "random_k": 4,
    "random_seed": 42,
...
def select_balanced_pairs(
    ...
    # Top matches
    top = (
        df.sort_values(["score", "resume_id"], ascending=[False, True])
    ...
    already = set(top[dedupe_col].tolist())

    # Borderline = around the middle score band
    remaining = df[~df[dedupe_col].isin(already)].copy()
    if not remaining.empty and borderline_k > 0:
        ...
        borderline = (
            candidate_band
            .sort_values(["distance_to_median", "resume_id"])
            .drop_duplicates(subset=[dedupe_col], keep="first")
            .head(min(borderline_k, len(candidate_band)))
            .copy()
        )
        borderline["selection_bucket"] = "borderline"
        selected_parts.append(borderline)
        already.update(borderline[dedupe_col].tolist())
    ...
    if not remaining.empty and random_k > 0:
        ...
        tail = tail.drop_duplicates(subset=[dedupe_col], keep="first").copy()

        choices = tail[dedupe_col].tolist()
        picked = set(rng.sample(choices, k=min(random_k, len(choices))))
        rnd = tail[tail[dedupe_col].isin(picked)].copy()
        rnd["selection_bucket"] = "random_negative"
        selected_parts.append(rnd)

    out = pd.concat(selected_parts, ignore_index=True)
    out = out.drop_duplicates(subset=[dedupe_col]).sort_values(
        ["selection_bucket", "score"], ascending=[True, False]
    )
...
def main() -> int:
    ...
    parser.add_argument("--config", default="smart_labels_config_experiment.json", help="Path to config JSON")
    ...
    for _, vac in vacancies.iterrows():
        ...
        for _, res in resumes.iterrows():
            ...
            pair_scores.append({
                ...
                "candidate_id": normalize_text(res.get("candidate_id", "")),
                "version_type": normalize_text(res.get("version_type", "")),
        ...
        if "candidate_id" not in scored_df.columns or scored_df["candidate_id"].astype(str).str.strip().eq("").all():
            dedupe_col = "resume_id"
